...
export const homeUnidades = defineType({
  ...
  fields: [
    ...
    defineField({
      name: 'cartoes',
      group: 'conteudo',
      title: 'Cartões de unidade',
      type: 'array',
      description:
        'Os três cartões como aparecem hoje na página inicial. Deixe vazio para usar as unidades escolhidas abaixo.',
      of: [
        defineField({
          name: 'cartaoUnidade',
          title: 'Cartão de unidade',
          type: 'object',
          fields: [
            defineField({ name: 'titulo', title: 'Nome da unidade', type: 'string' }),
            defineField({ name: 'endereco', title: 'Endereço', type: 'text', rows: 2 }),
            defineField({ name: 'caracteristica', title: 'Destaque (linha do check verde)', type: 'string' }),
            campoImagem('imagem', 'Foto da unidade', null),
            defineField({
              name: 'whatsapp',
              title: 'WhatsApp (só números, com DDI)',
              type: 'string',
              description: 'Exemplo: 555127970901',
            }),
            defineField({ name: 'linkDetalhes', title: 'Endereço da página da unidade', type: 'string' }),
          ],
    ...
    defineField({
      name: 'rotuloVisitas',
      group: 'conteudo',
      title: 'Selo sobre a foto',
      type: 'string',
      description: 'Vazio = "Visitas Diárias".',
    }),
    defineField({
      name: 'rotuloAgendar',
      group: 'conteudo',
      ...
      type: 'string',
      description: 'Vazio = "Agendar Visita".',
    }),
    defineField({
      name: 'rotuloWhatsapp',
      group: 'conteudo',
      ...
      type: 'string',
      description: 'Vazio = "WhatsApp".',
    }),
    defineField({
      name: 'rotuloDetalhes',
      group: 'conteudo',
      title: 'Texto do botão azul',
      type: 'string',
      description: 'Vazio = "Ver detalhes da unidade".',
    }),
    defineField({
      name: 'itens',
      group: 'conteudo',
      title: 'Ou escolher do cadastro de unidades',
      type: 'array',
      description: 'Só é usado quando não há cartões acima.',
...
export const homeServicos = defineType({
  ...
  fields: [
    ...
    defineField({
      name: 'cartoes',
      group: 'conteudo',
      title: 'Cartões de serviço',
      type: 'array',
      description:
        'Os cartões como aparecem hoje na página inicial. Deixe vazio para usar os serviços escolhidos abaixo.',
      of: [
        defineField({
          name: 'cartaoServico',
          ...
          fields: [
            defineField({ name: 'titulo', title: 'Título', type: 'string' }),
            defineField({ name: 'descricao', title: 'Texto', type: 'text', rows: 5 }),
            defineField({
              name: 'beneficios',
              title: 'Itens da lista',
              type: 'array',
              description: 'Até três linhas.',
              of: [{ type: 'string' }],
            }),
    ...
    defineField({
      name: 'itens',
      group: 'conteudo',
      title: 'Ou escolher do cadastro de serviços',
      type: 'array',
      description: 'Só é usado quando não há cartões acima.',
...
export const homeCtaFinal = defineType({
  ...
  fields: [
    ...
    defineField({
      name: 'rodape',
      group: 'conteudo',
      title: 'Texto acima dos links de apoio',
      type: 'text',
      rows: 2,
    }),
    defineField({
